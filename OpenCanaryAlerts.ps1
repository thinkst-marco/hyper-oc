...
function SetupVM {
    ...
    if ($vm -eq $null) {
        ...
        if (-not (Test-Path -Path $vmPath)) {
            ...
            Write-Error "Run: wget -OutFile oc.zip https://opencanary-hyperv-image.s3.eu-west-1.amazonaws.com/opencanary.zip; Expand-Archive opencanary.zip -DestinationPath .\"
            exit 1
        }
